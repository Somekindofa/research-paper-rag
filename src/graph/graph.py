...
def route_query(state: GraphState) -> str:
    ...
    Reads the routing_decision from state and returns the next node name.

    Args:
        state: Current graph state with routing_decision.

    Returns:
        Node name: "hyde" for retrieval path, "direct_answer" for direct path.
    """
...
def create_rag_graph() -> StateGraph:
    ...
    Flow:
    START -> preprocess -> router -> conditional:
        - retrieve path: hyde -> retrieve -> rerank -> generate -> END
        - direct path: direct_answer -> END
    ...
    graph.add_conditional_edges(
        "router", route_query, {"hyde": "hyde", "direct_answer": "direct_answer"}
    )
...
class RAGPipeline:
    ...
    def invoke(
        ...
        Args:
            ...
            force_retrieval: Force retrieval path (optional).
        ...
        if force_retrieval is not None:
            initial_state["force_retrieval"] = force_retrieval
